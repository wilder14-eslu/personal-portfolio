...
const Node = ({ title, subtitle, top, color, delay }) => (
  <motion.div
    ...
    style={{ left: '80%', top, borderColor: color }}
  ...
  return (
    <div className="relative w-full h-full min-h-[400px] flex items-center justify-center bg-blue-50/40 rounded-3xl border border-blue-100 overflow-hidden shadow-inner">
      ...
      <svg className="absolute inset-0 w-full h-full z-0" preserveAspectRatio="none" viewBox="0 0 100 100">
        <motion.path d="M 25,50 C 50,50 50,20 80,20" stroke="#00b4d8" strokeWidth="0.5" fill="none" strokeDasharray="1.5 1.5" variants={lineVariants} custom={1} initial="hidden" whileInView="visible" viewport={{ once: true }} />
        <motion.path d="M 25,50 L 80,50" stroke="#FF9900" strokeWidth="0.5" fill="none" strokeDasharray="1.5 1.5" variants={lineVariants} custom={2} initial="hidden" whileInView="visible" viewport={{ once: true }} />
        <motion.path d="M 25,50 C 50,50 50,80 80,80" stroke="#ff006e" strokeWidth="0.5" fill="none" strokeDasharray="1.5 1.5" variants={lineVariants} custom={3} initial="hidden" whileInView="visible" viewport={{ once: true }} />
      </svg>

      {/* Base Node (Statistics) */}
      <motion.div
        className="absolute left-[25%] top-[50%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center bg-gradient-to-br from-blue-600 to-cyan-500 text-white rounded-full shadow-xl shadow-blue-500/30 p-4 w-[110px] h-[110px] z-10 border-4 border-white"
